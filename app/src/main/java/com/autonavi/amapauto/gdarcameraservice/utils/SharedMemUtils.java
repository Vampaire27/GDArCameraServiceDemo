...
public class SharedMemUtils {
    ...
    /**
     * 设置frameId
     * 14~17:四个字节，contentSize
     *
     * @param header
     * @param frameId
     * @return
     */
    public static boolean setFrameId(byte[] header, int frameId) {
        if (header == null || header.length < HEADER_SIZE) {
            return false;
        }

        header[14] = (byte)(frameId >>> 24);
        header[15] = (byte)(frameId >>> 16);
        header[16] = (byte)(frameId >>> 8);
        header[17] = (byte)frameId;
        return true;
    }
     ...
     * @return
     */
    public static int getFrameId(byte[] header) {
        if (header == null || header.length < HEADER_SIZE) {
            return -1;
        }
        int result = 0;
        result = header[14] & 0xff;
        result = result << 8 | header[15] & 0xff;
        result = result << 8 | header[16] & 0xff;
        result = result << 8 | header[17] & 0xff;
        return result;
    }
